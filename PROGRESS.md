...
- **총 9개 PR 중 8개 완료**, PR 5(첫 가맹점 파서 — Trancy)는 사용자 fixture 확보 후 구현 진행 중
- 34개 단위 테스트 통과 / `ruff check`, `ruff format`, `mypy app` 모두 통과
...
| **5** | 🚧 **진행 중** | **첫 가맹점 파서 (Trancy). 사용자 fixture 확보 완료, 구현 진행 중.** | 5개 (예정) |
...
# 3. 테스트
uv run ruff check . && uv run mypy app && uv run pytest -v   # 34 passed
...
- **PR 5 블로커**: 사용자 fixture 도착 시점에 시작
