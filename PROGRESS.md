...
- **PR 0~9 (백엔드 MVP + force 옵션) 완료**, `origin/main` (`K-MG-0328/mailbook`, private) 에 push 완료
- **PR 10 완료**: 프론트엔드 PWA 변환 (Next.js 16 metadata API — `manifest.ts`/`icon.tsx`/`apple-icon.tsx`) + Tailscale HTTPS 단일 호스트 셋업 → iPhone Safari "홈 화면에 추가" 가능
- 42개 단위 테스트 통과 / `ruff check`, `ruff format`, `mypy app` 모두 통과 / 프론트 `npm run build && lint && typecheck` 통과
...
- 다음 단계: **Tailscale 설치 + 인증서 발급 + .env 채우기 + 실서버 검증** (아래 절차 참조)
...
| 10 | ✅ | 프론트엔드 PWA 변환 (`app/manifest.ts`/`icon.tsx`/`apple-icon.tsx`/layout metadata) + Tailscale HTTPS path-based same-origin 셋업 — iPhone/Mac 홈 화면 설치 가능 | 빌드 검증 (라우트 자동 생성) |
...
두 가지 시나리오 — 로컬 개발 / Tailscale PWA 운영. 처음 셋업은 로컬→Tailscale 순서로 진행 권장.

### 시나리오 A: 로컬 개발 (맥북에서만 사용)
...
3. **`.env` 채우기** (`Mailbook-backend/.env`):
   ```
   ...
   ```
4. Redis / PostgreSQL 동작 확인 + `uv run alembic upgrade head`
...
# Frontend
cd ../Mailbook-frontend && npm install && npm run dev   # http://localhost:3000

# OAuth → sync → verify
curl http://localhost:8000/api/v1/auth/gmail/start
curl -X POST http://localhost:8000/api/v1/sync
curl 'http://localhost:8000/api/v1/payment-events?matched=false'
curl -X POST 'http://localhost:8000/api/v1/transactions/resolve-solo?force=true'
curl http://localhost:8000/api/v1/transactions
```

---

### 시나리오 B: Tailscale PWA (iPhone + Mac 양쪽 사용)

#### 사전 준비

1. **Tailscale 설치** (사용자 직접 — sudo 비밀번호 필요)
   - 맥북: `brew install --cask tailscale` 후 메뉴바 앱에서 로그인
   - 아이폰: App Store → Tailscale 설치 → 같은 계정 로그인
   - 양 디바이스가 같은 tailnet 에 들어왔는지 확인:
     ```bash
     tailscale status
     ```
...
2. **HTTPS 인증서 발급** (1회):
   ```bash
   # 호스트네임 확인 (맥북의 tailnet 이름)
   tailscale status --self | head -1
   # 예시 출력: 100.x.y.z  gimmingyu-mbp gimmingyu@... macOS active

   # Let's Encrypt 인증서 발급 (자동)
   sudo tailscale cert <hostname>.<tailnet>.ts.net
   ```

3. **Tailscale serve — 단일 HTTPS 호스트로 path 라우팅**:
   ```bash
   # /api/* → backend (8000)
   sudo tailscale serve --bg --https=443 --set-path=/api http://localhost:8000
...
4. **Google Cloud Console** — OAuth Client 의 Authorized redirect URIs 에 추가:
   ```
   https://<hostname>.<tailnet>.ts.net/api/v1/auth/gmail/callback
   ```

5. **`.env` 갱신**:
   - `Mailbook-backend/.env`:
     ```
     GMAIL_OAUTH_REDIRECT_URI=https://<hostname>.<tailnet>.ts.net/api/v1/auth/gmail/callback
     CORS_ALLOWED_FRONTEND_URL=https://<hostname>.<tailnet>.ts.net
     ```
   - `Mailbook-frontend/.env.local`:
     ```
     NEXT_PUBLIC_API_BASE_URL=https://<hostname>.<tailnet>.ts.net
     ```

6. **양쪽 서버 띄우기** (별도 터미널):
   ```bash
   cd Mailbook-backend && uv run uvicorn main:app --reload --port 8000 --host 0.0.0.0
   cd Mailbook-frontend && npm run build && npm start   # 3000 포트
   ```

#### 검증 단계
1. **맥북 Safari**: `https://<hostname>.<tailnet>.ts.net` 접속 → "M" 아이콘 placeholder 페이지 확인
2. **아이폰 Safari**: 같은 URL 접속 → 공유 → "홈 화면에 추가" → 아이콘 풀스크린 동작 확인
3. **OAuth/sync**:
   ```bash
   curl https://<hostname>.<tailnet>.ts.net/api/v1/auth/gmail/start
   # auth_url 을 아이폰 브라우저에서 열어 로그인
   curl -X POST https://<hostname>.<tailnet>.ts.net/api/v1/sync
   curl -X POST 'https://<hostname>.<tailnet>.ts.net/api/v1/transactions/resolve-solo?force=true'
   ```

> `force=true` 는 24h 타임아웃을 우회하는 검증/관리자 전용 채널. 운영 SyncPipeline 호출부는 여전히 24h 컷오프 사용.
...
- **Tailscale ACL 기본값**: 본인 계정의 모든 디바이스만 접근 가능 (외부 인터넷 노출 없음). 가족 등에게 tailnet 초대장 보내면 그 사람도 접근 가능해지니 주의
- **맥북 sleep**: Tailscale 은 sleep 에서도 동작하지만 완전 종료 시엔 아이폰에서 접속 불가. 평소 sleep 권장
- **iOS PWA Service Worker 미사용**: 가계부는 항상 fresh 데이터 필요. 오프라인 셸 필요해지면 후속 PR
- **path 충돌 회피**: 프론트엔드는 `/api/...` 경로 사용 금지 — Tailscale serve 가 backend 로 라우팅함
